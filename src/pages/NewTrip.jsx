...
export default function NewTrip() {
  ...
  // working draft of the trip we're building
  const [tripData, setTripData] = useState({
    ...
    ai_suggestions: null, // { missing:[], climate:[], purpose_specific:[] }
  ...
  // Make any backend shape safe for the UI (arrays of strings).
  function normalizeAISuggestions(raw) {
    const toArray = (val) => {
      if (!val) return [];
      if (Array.isArray(val)) return val;
      if (typeof val === "string") return [val];
      // Sometimes servers send an object (e.g., {0:"x",1:"y"}) -> coerce values
      if (typeof val === "object") return Object.values(val).flat().map(String);
      return [];
    };

    // Case A: Newer backend: { suggestions:[ {text, category}, "string", ... ] }
    const suggestionsArray = toArray(raw?.suggestions).map((s) =>
      typeof s === "string" ? { text: s, category: undefined } : s
    );

    const missingFromSug = suggestionsArray
      .filter((s) => (s.category || "").toLowerCase() === "missing")
      .map((s) => s.text?.toString() ?? "");

    const climateFromSug = suggestionsArray
      .filter((s) => (s.category || "").toLowerCase() === "climate")
      .map((s) => s.text?.toString() ?? "");

    const purposeFromSug = suggestionsArray
      .filter((s) => (s.category || "").toLowerCase().includes("purpose"))
      .map((s) => s.text?.toString() ?? "");

    // Case B: Older backend: { missing:[], climate:[], purpose_specific:[] } possibly as strings/objects
    const missingTop = toArray(raw?.missing).map(String);
    const climateTop = toArray(raw?.climate).map(String);
    const purposeTop = toArray(raw?.purpose_specific || raw?.purposeSpecific).map(String);

    // Merge, de-dupe, and filter empties
    const dedupe = (arr) => [...new Set(arr.filter(Boolean))];

    return {
      missing: dedupe([...missingTop, ...missingFromSug]),
      climate: dedupe([...climateTop, ...climateFromSug]),
      purpose_specific: dedupe([...purposeTop, ...purposeFromSug]),
    };
  }
  ...
  async function fetchAISuggestions() {
    ...
    setError(null);
    try {
      const raw = await getAiSuggestions({
      ...
      const normalized = normalizeAISuggestions(raw || {});
      updateTripData({ ai_suggestions: normalized });
    } catch (err) {
      console.error(err);
  ...
  async function fetchWeightData() {
    ...
    setError(null);
    try {
      const weightData = await getWeights({ items: tripData.items });
      // weightData: { items: [{ aiWeight, ... }], totalG }
      ...
      const totalWeightKg = Number((weightData?.totalG || 0) / 1000);

      let optimizationResult = null;
      if (totalWeightKg > Number(tripData.airline_limit || 0)) {
  ...
  async function fetchPackingStrategy() {
    setLoading(true);
    setError(null);
    try {
  ...
  async function handleNext() {
    ...
    // Final step => save trip to backend
    if (currentStep === 5) {
  ...
  //
  // SAVE FINAL TRIP TO BACKEND + NAVIGATE TO SUMMARY
  //
        ...
        <div className="max-w-4xl mx-auto px-6 py-4">
          <div className="flex items-center gap-4 mb-2">
            <Button
              ...
              size="icon"
              onClick={() =>
                currentStep === 1 ? navigate(createPageUrl("Home")) : handleBack()
              }
              disabled={loading}
            >
            ...
            {loading ? (
              ...
              <>
                ...
                {tripData.optimization &&
                  tripData.total_weight > tripData.airline_limit && (
