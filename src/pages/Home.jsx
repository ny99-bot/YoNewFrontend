...
// util to build URLs like createPageUrl('NewTrip')
import { createPageUrl } from '@/utils';

// our local backend client
import { getUid, listTrips } from '../apiClient';
...
export default function Home() {
  ...
  // who is logged in (temp fake user for now)
  ...
  useEffect(() => {
    async function load() {
      try {
        const uid = getUid(); // "demo-user" for now
        const userFirstName = uid === "demo-user" ? "Traveler" : uid;
        setUserName(userFirstName);

        const tripsList = await listTrips(uid);
        setTrips(tripsList);
      } catch (err) {
